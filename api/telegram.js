...
module.exports = apiHandler(async (req, res) => {
  if (req.method === 'GET') return res.json({ status: 'active', mode: 'webhook' });
  ...
  const task = require('../services/telegramBot').handleUpdate(update);
  processedUpdates.set(key, task);
  try { await task; }
  catch (error) { processedUpdates.delete(key); throw error; }
