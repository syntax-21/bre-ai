...
module.exports = apiHandler(async (req, res) => {
  ...
  if (!isAdmin && (req.method === 'POST' || token)) {
    ...
    throw httpError(401, (process.env.ADMIN_PASSWORD || cfg.adminPassword) ? 'Unauthorized: Password admin diperlukan' : 'Admin belum dikonfigurasi. Atur ADMIN_PASSWORD di environment.');
  }
  if (req.method === 'GET') {
