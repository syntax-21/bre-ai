...
async function saveAllConfig() {
  ...
  const payload = {
    ...
    cacheTTL: Math.max(1, Math.min(86400, parseInt(getVal('cfgCacheTTL'), 10) || 3600)),
    blacklist: getVal('cfgBlacklist').split('\n').map(w => w.trim()).filter(Boolean),
    systemPrompt: getVal('cfgPrompt'),
    temperature: Math.max(0, Math.min(2, parseNum(getVal('cfgTemp'), 0.7))),
    topP: Math.max(0, Math.min(1, parseNum(getVal('cfgTopP'), 1.0))),
    requireAuth: false,
    frequencyPenalty: Math.max(-2, Math.min(2, parseNum(getVal('cfgFreqPenalty'), 0.0))),
    presencePenalty: Math.max(-2, Math.min(2, parseNum(getVal('cfgPresPenalty'), 0.0))),
    maxTokens: Math.max(1, Math.min(32768, parseInt(getVal('cfgMaxTokens'), 10) || 16384)),
    defaultStyle: getVal('cfgDefaultStyle') || 'santai',
    rateLimitMax: Math.max(1, Math.min(100, parseInt(getVal('cfgRateMax'), 10) || 5)),
    rateLimitWindow: Math.max(1, Math.min(3600, parseInt(getVal('cfgRateWin'), 10) || 30)),
...
telegramDomain: getVal('cfgTelegramDomain').trim(),
    ...
    motivationEnabled: getCheck('cfgMotivationEnabled'),
    motivationTimes: [getVal('cfgMotivationTime1') || '08:00'].filter(Boolean),
    motivationCustom: getVal('cfgMotivationCustom'),
    transcriptionEnabled: getCheck('cfgTranscriptionEnabled'),
    transcriptionEndpoint: getVal('cfgTranscriptionEndpoint').trim(),
    transcriptionKey: getVal('cfgTranscriptionKey').trim(),
    transcriptionModel: getVal('cfgTranscriptionModel').trim() || 'whisper-1',
    transcriptionLanguage: getVal('cfgTranscriptionLanguage').trim() || 'auto',
    telegramUsers,
